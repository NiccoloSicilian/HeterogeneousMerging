...
def print_table(title, names, accs, results):
    col_keys = [k for k in results[0] if k != 'avg']
    # Sort by accuracy descending
    rows = sorted(zip(names, accs, results), key=lambda x: x[1], reverse=True)
    print(f"\n{'=' * 20} {title} {'=' * 20}")
    header = f"{'subset':<45} {'acc':>8}" + "".join(f"{k:>20}" for k in col_keys) + f"{'avg':>20}"
    print(header)
    print("-" * len(header))
    for name, acc, result in rows:
        row = f"{name:<45} {acc:>8.2f}"
        for k in col_keys:
            row += f"{result.get(k, 0):>20.6f}"
        row += f"{result['avg']:>20.6f}"
        print(row)


if ANALYSIS_CONFIG is not None:

    # Group analyses by experiment name
    groups = {}
    for analysis in ANALYSIS_CONFIG["analyses"]:
        load_params = analysis["load_params"]
        load_method = analysis["load_method"]
        experiment = analysis["experiment"]
        experiment_param = analysis['experiment_param']
        acc = analysis.get('acc', 0.0)
        data = load_method(load_params)
        result = experiment(data, experiment_param)

        exp_name = experiment.__name__
        name = load_params.get('source', load_params.get('pretrained', ''))

        if exp_name not in groups:
            groups[exp_name] = {'names': [], 'accs': [], 'results': []}
        groups[exp_name]['names'].append(name)
        groups[exp_name]['accs'].append(acc)
        groups[exp_name]['results'].append(result)

    for title, group in groups.items():
        print_table(title, group['names'], group['accs'], group['results'])
